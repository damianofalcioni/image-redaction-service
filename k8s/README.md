...
```bash
docker build -t ghcr.io/YOUR_ORG/image-redaction-service:0.4.0 .
docker push ghcr.io/YOUR_ORG/image-redaction-service:0.4.0
```
...
```bash
kubectl kustomize k8s | sed 's#ghcr.io/YOUR_ORG/image-redaction-service:0.4.0#YOUR_REGISTRY/image-redaction-service:0.4.0#g' | kubectl apply -f -
```
